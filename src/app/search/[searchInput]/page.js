...
export default function SearchResultPage({ params }) {
  ...
  */
  const moviename = params.searchInput;

  const PSI = moviename.replaceAll("%20", " ");
  //const lowercasePSI = processedSearchInput.toLowerCase();
  //const lowercaseMTT = movieDB.toLowerCase();
  const filteredMovies = movieDB.filter((movie) =>
    movie.title.toLowerCase().includes(PSI.toLowerCase())
  );
